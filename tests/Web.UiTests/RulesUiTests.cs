...
// fully-qualify component type to avoid conflict with System.Index

namespace Web.UiTests
{
    public class RulesUiTests : BunitContext
    {
        [Fact]
        public void Index_ShowsCategoryAndEnvelopeNames_And_DeletesWhenConfirmed()
        {
            ...
            // Intercept JS confirm to return true (match any arguments)
            JSInterop.Setup<bool>("confirm", _ => true).SetResult(true);

            // Act
            var comp = Render<LocalFinanceManager.Web.Components.Pages.Rules.Index>();

            // Assert: names are displayed
            Assert.Contains("Food", comp.Markup);
            Assert.Contains("Dining", comp.Markup);
